...
export class DatasetsService {
   ...
   * @param options
   */
  getFieldsForWildcard = async (options: BaseGetFieldsOptions = {}) => {
   ...
   * @param options
   */
  getFieldsForDataset = async (
    dataset: Dataset | DatasetSpec,
    options: BaseGetFieldsOptions = {}
  ) =>
  ...
  private addFormatsToFields = (
    fieldSpecs: BaseFieldSpec[],
    fieldFormatMap: BaseFieldFormatMap
  ) => {
    Object.entries(fieldFormatMap).forEach(([fieldName, value]) => {
      const field = fieldSpecs.find((fld: BaseFieldSpec) => fld.name === fieldName);
      if (field) {
        field.format = value;
      }
    });
   ...
   * @param fields
   */
  fieldArrayToMap = (fields: BaseFieldSpec[]) =>
  ...
  savedObjectToSpec = (savedObject: SavedObject<DatasetAttributes>): DatasetSpec => {
    ...
    const parsedFields: BaseFieldSpec[] = fields ? JSON.parse(fields) : [];
  ...
  get = async (id: string, onlyCheckCache: boolean = false): Promise<Dataset> => {
    ...
    const parsedFieldFormats: BaseFieldFormatMap = savedObject.attributes.fieldFormatMap
