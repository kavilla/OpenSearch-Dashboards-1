...
function migratePre61PanelToLatest(
  ...
  if (panel.col === undefined || panel.row === undefined) {
    throw new Error(
      i18n.translate('dashboardNew.panel.unableToMigratePanelDataForSixOneZeroErrorMessage', {
        defaultMessage:
...
function migrate610PanelToLatest(
  ...
  (['w', 'x', 'h', 'y'] as Array<keyof GridData>).forEach((key) => {
    if (panel.gridData[key] === undefined) {
      throw new Error(
        i18n.translate('dashboardNew.panel.unableToMigratePanelDataForSixThreeZeroErrorMessage', {
          defaultMessage:
